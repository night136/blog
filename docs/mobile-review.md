...
- 🔄 **2026-09-19 更新：阈值 380 → 400**（起因与实测见 `optimization-audit-2026-09-16.md`
  §二十二 / §二十三）。实测 381–393px 的每列只有 148.5–155px，正是本条自述"~150px 以下
  更难读"的区间却没被兜住（381px 处 149px 已跌破 150）。抬到 400 后 iPhone 12–15 常规
  （390/393）走单列（便签 318/321px），400 以上两列每列 ≥158.5px。
  - ⚠️ 上面「380 块…已入断言」这句里的**具体数字已过时**；顺序约束本身仍然有效。
  - 守护同步改为**不锁阈值数字**（守"存在回落 + 顺序 + 量级 280–640"）；
    渲染侧的不变量「任何视口下便签不窄于 150px」交给真浏览器探针
    `scripts/audit-guestbook-width.mjs`（它自动探测边界，阈值再改也不用动探针）。
- 🔄 **2026-09-19 **同日再次**调整：400 → 320**（所有者拍板，方向与上一段相反：
  优先「尽量少单列」）。≤320 才单列 ⇒ 321–383px 回到两列、只有 118–149px，
  390/393 回到 153/154.5px。**「便签 ≥150px」⇔ 阈值 ≥384，二者互斥**，
  这次是显式拿可读下限换少单列。详见 `optimization-audit-2026-09-16.md` §二十四。
  - ⇒ 上一段「不窄于 150px」那条渲染侧不变量**已随之失效**（前提被取消，不是放松断言）：
    探针改为 **118px 回退保护**（= 阈值 320 下已实测的代价，最窄 119px @321px）。
...
- `verify-mobile-guards.mjs` 从 50 → **65 项**，新增 `[10]` 块共 15 条：
  ...
  便签 `minmax(0,1fr)` / 单列回落（当时是 ≤380，2026-09-19 抬到 400）/ 回落块在 640 之后；
  字体 3 档且含 900 不含 600；logo 残留。
