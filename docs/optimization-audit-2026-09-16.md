...
| PBKDF2 渐进升级 | 迭代数**写进 hash**：`pbkdf2$<iter>$<salt>$<hash>`；老的 `salt:hash` 按 100000 校验；校验通过后顺手用新迭代数重写 | 🚫 绝不能直接改常量（会锁死全部现存密码）。⚠️ 连**假哈希**也必须用同一个 `targetIterations(env)` —— 用老格式的假哈希会让"用户不存在"明显更快，把时序侧信道又打开。🔴 **默认迭代数受边缘 CPU 预算硬约束：只能是 100000，不能按教科书建议值上调** —— 见 §十六（这批曾提到 210000，把线上注册打挂了） |
...
## 十六、2026-09-17 事故：PBKDF2 迭代数 210000 超出边缘 CPU 预算，线上**注册全挂** —— ✅ 已修

用户报「注册失败，请稍后重试」。根因是**上一批把 PBKDF2 默认迭代数从 100000 提到 210000**，
而 210000 超出了 Cloudflare Pages 免费档的 CPU 预算 —— 每一次注册请求都被平台终止。
...
**① 受平台硬约束的常量，不能按教科书建议值调。**
OWASP 建议 PBKDF2-HMAC-SHA256 用 210000~600000 —— 那是**在你能控制 CPU 预算的前提下**。
这里是 10 ms。**"更安全"的建议值在这里会让功能完全不可用**，而失败形态是"整条请求被终止"，
不是"变慢"，所以压测不出来、只能靠真机。凡是"调大一个数字"的改动，先问**它的上限由谁决定**。
...
- ⚠️ **强度确实退回了 100000**。要满足 OWASP 建议量级需升级到 Workers **Paid** 档
  （CPU 预算 30 s），届时用 `PBKDF2_ITERATIONS` 灰度上调，**先真机跑通注册 + 登录**再全量。
