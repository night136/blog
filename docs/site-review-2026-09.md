...
> **后续已处理项**（2026-09-15）：**爬虫拿不到文章正文**。
> 这一项原本没进上表（它是功能缺失而非代码缺陷），但实际影响最大 —— 百度蜘蛛实测打开文章
> 只能看到一个空的 `#postDetail`，**正文零收录**；顺带还有 `<title>` 未替换、双 canonical 两个连带问题。
> 已按 dynamic rendering 修好，详见 [`docs/seo-crawler-render.md`](./seo-crawler-render.md)。
>
> 与 #17（`# ` 被渲染成 `<h2>`、页面缺 `h1`）的关系：本次**没有**改标题层级 ——
> `index.html` 侧边栏的品牌名已占用了一个 `h1`，要理清「文章页到底哪个该是 `h1`」
> 需要连着侧边栏语义一起改（并把 `.post-detail h2` 的样式同步到 h1），属于独立改动，留给后续决策。
> 正文能被抓取之后，这一项对收录的影响已经小了很多。
